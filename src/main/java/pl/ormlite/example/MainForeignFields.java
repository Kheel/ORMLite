...
public class MainForeignFields {
    ...
    public static void main (String[] args) throws SQLException, IOException, ParseException {
       ...
       // daoAuthor.refresh(book.getAuthor()); // // mozemy sie pozbyc po dopisaniu autorefresh w Book
        ...
        queryBuilder.where().eq("TITLE","Wiedzmin");
        PreparedQuery<Book> prepare = queryBuilder.prepare();
        List<Book> resultQuery = dao.query(prepare);
        ...
        List<Book> result2 = dao.query(dao.queryBuilder().
                where().
                eq("TITLE", "Pieśń Lodu i Ognia").
                //and()
                //eq.("PRICE", xx.xx).prepare()); //bardziej zlozone zapytanie
                prepare());
        result2.forEach(e->{
            System.out.println();
            System.out.println(e);
        ...
        updateBuilder.updateColumnValue("DESCRIPTION", "Wielki swiat stworzony przez Tolkiena");
